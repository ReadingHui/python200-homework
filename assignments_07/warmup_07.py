    ...
    'function': {
        ...
        'parameters': {
            ...
            'properties': {
                "celsius": {
                    "type": "number",
                    "description": "The temperature in degrees Celsius to be converted."
                }
            },
            'required': ["celsius"],
        },
...
# Ans: It will not trigger a tool call, as the model is only equipped with the function get_current_time, it is not used to convert Celsius to Fahrenheit.
# 2. How many API calls will be made to answer this query?
# Ans: 1 API call, as no tools will be called, the first response will be the final response.
print(f"Result of the run: {run_agent("Convert 100 degrees Celsius to Fahrenheit")}")
print()

# Q3 - Updating run_agent function
print("=== Q3 ===")
def get_current_time() -> str:
    '''Return the current local time as a formatted string.'''
    return datetime.now().strftime('%Y-%m-%d %H:%M:%S')
...
tools.append(c_to_f_schema)
print('Tools list defined with two tools: celsius_to_fahrenheit, get_current_time')


def run_agent(user_prompt: str) -> str:
    '''Run a minimal ReAct-style agent for a single user prompt.'''

    SYSTEM_PROMPT = '''You are a simple assistant that can either tell the current time or converts a temperature in Celsius to Fahrenheit.
                     Use the tool get_current_time whenever a user asks about the time, and celsius_to_fahrenheit whenever a user asks about the conversion.'''
...
tools_schema = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Compute the Pearson correlation from the active CSV. Name of the columns must be provided.",
            "parameters": {
                "type": "object",
                "properties": {
                    "col1": {"type": "string", "description": "Column name for the first column."},
                    "col2": {"type": "string", "description": "Column name for the second column."},
                },
                "required": ["col1", "col2"],
            },
        },
...
# Q4
# compute_correlation added to CsvManager, tool_node and tool_schema
...
@tool
def compute_correlation(col1: str, col2: str) -> str | dict:
        """
        Compute the Pearson correlation between two columns in the loaded DataFrame.
        Args:
...
print("=== Q8 ===")
print("Response from tool agent:")
print(response_tool)
print()
