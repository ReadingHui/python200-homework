...
I would need to change the `OPEN_MATEO_CONFIG` to pull data from previous day's forecast instead of the preset range in 2023. That should be enough to keep the pipeline running smoothly on previous day's weather. However, it is also a good idea to update the prediction model once in a while, say every quarter or so, with the new and updated weather data.